...
# Define output file
OUTPUT_FILE="plain_run_results.txt"
NUM_RUNS=25
SENDERS_LIST=(8 16 32 64 128 256 512 1024)

# Clear output file before starting
echo "Plain Intersection Experiment Results" > $OUTPUT_FILE
echo "=====================================" >> $OUTPUT_FILE
...
for NUM_SENDERS in "${SENDERS_LIST[@]}"; do
    ...
    for ((i=1; i<=NUM_RUNS; i++)); do
        START_TIME=$(date +%s.%N)
        OUTPUT=$(./run_plain $NUM_SENDERS 2>&1)
        END_TIME=$(date +%s.%N)

        # Extract the first value as intersection time
        INTERSECTION_TIME=$(echo "$OUTPUT" | awk '{print $1}')
